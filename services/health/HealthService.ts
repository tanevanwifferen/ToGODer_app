...
export class HealthService {
  ...
  static async getHealthDataSummerized(): Promise<string>{
    ...
    const weeklyMentalHealthStats = await HealthService.getWeeklyMentalHealthStats();
    const monthlyMentalHealthStats = await HealthService.getMonthlyMentalHealthStats();
    ...
    let toreturn = `excercised average per day this week: ${weeklyExerciseStats.averageMinutes}, \
     this month: ${monthlyExerciseStats.averageMinutes} minutes.`; 

    if(weeklyMentalHealthStats.averageValence !== 0 && monthlyMentalHealthStats.averageValence !== 0){
      toreturn += `average weekly mood (0-10) ${weeklyMentalHealthStats.averageValence * 10} \
              and average monthly mood is ${monthlyMentalHealthStats.averageValence * 10} \
      `;
    }

    if(weeklySleepStats.averageTimeSpentInBed !== 0 && monthlySleepStats.averageTimeSpentInBed !== 0) {
      toreturn += `\nSleep stats:\n\
      Weekly: ${Math.round(weeklySleepStats.averageTimeSpentInBed / 60)} hours in bed, typically at ${weeklySleepStats.averageGoingToBedtime}\n\
      Monthly: ${Math.round(monthlySleepStats.averageTimeSpentInBed / 60)} hours in bed, typically at ${monthlySleepStats.averageGoingToBedtime}`;
    }
